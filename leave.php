...
if($_SESSION['ROLE']==1){ 
	$sql="select `leave`.*, drivers.name,drivers.id as eid from `leave`,drivers where `leave`.drivers_id=drivers.id order by `leave`.id desc";
}else{
	$eid=$_SESSION['USER_ID'];
	$sql="select `leave`.*, drivers.name ,drivers.id as eid from `leave`,drivers where `leave`.drivers_id='$eid' and `leave`.drivers_id=drivers.id order by `leave`.id desc";
}
